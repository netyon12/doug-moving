...
def listagem_solicitacoes():
    ...
    if empresa_id_usuario:
        empresas = query_tenant(Empresa).filter_by(id=empresa_id_usuario).all()
    else:
        empresas = query_tenant(Empresa).all()

    # Filtrar plantas baseado no perfil
    if planta_id_usuario:
        plantas = query_tenant(Planta).filter_by(id=planta_id_usuario).all()
    elif empresa_id_usuario:
        plantas = query_tenant(Planta).filter_by(empresa_id=empresa_id_usuario).all()
    else:
        plantas = query_tenant(Planta).all()

    blocos = query_tenant(Bloco).all()

    # Buscar supervisores (apenas para admin)
    if current_user.role in ['admin', 'operador']:
        supervisores = query_tenant(Supervisor).all()
    else:
        supervisores = []
...
def dados_listagem_solicitacoes():
    ...
    try:
        ...
        # Construir query
        query = query_tenant(Solicitacao)
        ...
        for sol in solicitacoes:
            # Usar relacionamentos já carregados (eager loading)
            colaborador = sol.colaborador
            empresa = sol.empresa
            planta = sol.planta
            bloco = sol.bloco
            supervisor = sol.supervisor
...
def conferencia_viagens():
    ...
    colaboradores = query_tenant(Colaborador).all()
    ...
    try:
        ...
        if current_user.role == 'supervisor':
            ...
            if supervisor_id_usuario:
                ...
                solicitacoes_supervisor = query_tenant(Solicitacao).filter_by(
        ...
        for viagem in viagens:
            if viagem.colaboradores_ids:
                try:
                    ...
                    todos_col_ids.update(col_ids)
                except:
                    pass

        # 1 query para buscar todos os colaboradores
        colaboradores_dict = {}
        if todos_col_ids:
            cols = query_tenant(Colaborador).filter(
                Colaborador.id.in_(todos_col_ids)).all()
            colaboradores_dict = {col.id: col for col in cols}
        ...
        for viagem in viagens:
            ...
            planta = viagem.planta
            bloco = viagem.bloco
            motorista = viagem.motorista

            # Buscar colaboradores (otimizado - lookup em memória)
            colaboradores = []
            if viagem.colaboradores_ids:
                try:
                    col_ids = json.loads(viagem.colaboradores_ids)
                    for col_id in col_ids:
                        col = colaboradores_dict.get(col_id)
                        if col:
...
def dados_conferencia_motoristas():
    ...
    try:
        ...
        for viagem in viagens:
            ...
            # Buscar colaboradores e bairros (otimizado - lookup em memória)
            colaboradores = []
            bairros = []
            if viagem.colaboradores_ids:
                try:
...
def plantas_por_empresa(empresa_id):
    ...
    try:
        plantas = query_tenant(Planta).filter_by(empresa_id=empresa_id).all()
        return jsonify({
            'success': True,
...
def exportar_pdf(tipo):
    ...
    try:
        ...
        # ✅ ADICIONAR TOTALIZADORES PARA CONFERÊNCIA DE VIAGENS
        if tipo == 'viagens':
            # Calcular totais
            total_viagens = len(dados)
            total_valor = sum(
                float(item.get('valor', 0)) if item.get('valor') else 0
                for item in dados
            )
            ...
            totalizadores_data = [
                ...
                ['Valor Total:', f"R$ {total_valor:,.2f}".replace(
                    ',', 'X').replace('.', ',').replace('X', '.')]
            ]

            totalizadores_table = Table(
                totalizadores_data, colWidths=[2*inch, 1.5*inch])
            totalizadores_table.setStyle(TableStyle([
            ...
            elements.append(totalizadores_table)
