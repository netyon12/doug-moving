...
def solicitacoes():
    ...
    if current_user.role == 'gerente':
        ids_supervisores = [s.id for s in query_tenant(Supervisor).filter_by(
 ...
 # ===== FILTRO PADRÃO DO MÊS ATUAL =====
    ...
    if current_user.role in ['admin', 'operador']:
        supervisores_filtro = query_tenant(Supervisor).order_by(Supervisor.nome).all()
        blocos_filtro = query_tenant(Bloco).order_by(Bloco.codigo_bloco).all()
    elif current_user.role == 'gerente':
        supervisores_filtro = query_tenant(Supervisor).filter_by(
            gerente_id=current_user.gerente.id).order_by(Supervisor.nome).all()
        blocos_filtro = query_tenant(Bloco).join(Empresa).filter(
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if current_user.role == 'supervisor':
                ...
                planta = query_tenant(Planta).get(planta_id)
                if not planta:
            ...
            def buscar_turno_id(nome_turno, planta_id):
                if not nome_turno or nome_turno == 'Não definido':
                    return None
                turno = query_tenant(Turno).filter_by(
            ...
            def buscar_valores(bloco_id, turno_id):
                ...
                bloco = query_tenant(Bloco).get(bloco_id)
                turno = query_tenant(Turno).get(turno_id)
            ...
            for i, colab_id in enumerate(colaborador_ids):
                ...
                colaborador = query_tenant(Colaborador).get(colab_id)
                if not colaborador:
                    continue
                ...
                if tipo_corrida == 'entrada':
                    ...
                    if i < len(horarios_entrada) and horarios_entrada[i]:
                        ...
                        entrada_existente = query_tenant(Solicitacao).filter(
                        ...
                        if not entrada_existente:
                            ...
                            get_tenant_session().add(solicitacao_entrada)
                    ...
                    if i < len(horarios_saida) and horarios_saida[i]:
                        ...
                        saida_existente = query_tenant(Solicitacao).filter(
                            Solicitacao.colaborador_id == colab_id,
                        ...
                        if not saida_existente:
                            ...
                            get_tenant_session().add(solicitacao_saida)
                            get_tenant_session().flush()  # Para obter o ID
                            solicitacoes_ids_criadas.append(
              ...
              #      desligamento_existente = query_tenant(Solicitacao).filter(
              #          Solicitacao.colaborador_id == colab_id,
              ...
              #      continue
                ...
                get_tenant_session().add(solicitacao)
                get_tenant_session().flush()  # Para obter o ID
...
def visualizar_solicitacao(id):
    ...
    solicitacao = get_or_404_tenant(Solicitacao, id)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if turno_principal_id and solicitacao.bloco_id:
                bloco = query_tenant(Bloco).get(solicitacao.bloco_id)
                turno = query_tenant(Turno).get(turno_principal_id)
...
def excluir_solicitacao(id):
    try:
        ...
        get_tenant_session().delete(solicitacao)
        get_tenant_session().commit()
