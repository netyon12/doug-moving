...
def log_audit(
    ...
    try:
        ...
        if request:
            ...
            user_agent = request.headers.get('User-Agent', '')[:255]  # Limita a 255 chars
...
def get_changes_dict(old_obj, new_obj, fields):
    ...
    for field in fields:
        ...
        old_value = getattr(old_obj, field, None) if hasattr(old_obj, field) else old_obj.get(field)
        new_value = getattr(new_obj, field, None) if hasattr(new_obj, field) else new_obj.get(field)
